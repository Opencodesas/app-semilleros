...
import {
   getMunicipalitiesUserDisciplines,
   getUserRegionsMunicipalities,
} from "@/composables/getMonitorByMunicipality"
...
const usersDisciplinesByMunicipality =
   ref<{ discipline: string; discipline_id: number }[]>()
         ...
         .then((response) => {
            if (response) {
               if (response.status >= 200 && response.status <= 300) {
                  ...
                  setLoading(true)
                  router
                     .push({ name: "methodologist_visits.index" })
                     .finally(() => {
                        setLoading(false)
                     })
               }
