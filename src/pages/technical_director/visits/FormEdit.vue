	...
	<div class="p-5 mt-5 intro-y box">
		...
		<div class="grid grid-cols-1 md:grid md:grid-cols-2 gap-6 justify-evenly">
			...
			<CommonInput :disabled="diableElements" type="time" label="Hora  *" name="hour_visit" v-model="form.hour_visit"
				:validator="v$" />
			<CommonSelect :disabled="diableElements" label="Municipio *" name="municipality_id" class="cursor-pointer"
			...
			<CommonSelect :disabled="diableElements" label="Monitor *" name="monitor_id" class="cursor-pointer"
				v-model="form.monitor_id" :validator="v$" :options="monitorList" />
			<CommonSelect :disabled="diableElements" label="Disciplinas *" name="discipline_id" class="cursor-pointer"
			...
			<CommonInput :disabled="diableElements" type="number" min="0" placeholder="Ingresar el numero de beneficiarios"
				label="Cobertura de benificiario *" name="beneficiary_coverage" v-model="form.beneficiary_coverage"
				:validator="v$" />
			<CommonSelect :disabled="diableElements" label="Cumple con el desarrollo tecnico del mes *" name="technical"
				class="cursor-pointer" v-model="form.technical" :validator="v$" :options="evaluationList" />
			<CommonSelect :disabled="diableElements" label="Apoyo a eventos *" name="event_support_id"
				class="cursor-pointer" v-model="form.event_support_id" :validator="v$" :options="event_supportList" />
		</div>
		<div class="mt-6 intro-y">
			<CommonTextarea :disabled="diableElements" label="Descripcion *" rows="5" placeholder="Ingrese las Descripcion"
				name="description" v-model="form.description" :validator="v$" />
		</div>
		<div class="mt-6 intro-y">
			<CommonTextarea :disabled="diableElements" label="Observaciones *" rows="5"
		...
		<div class="p-5 mt-6 intro-y">
			<CommonFile v-if="parseInt(form.status_id) == 4" :validator="v$" v-model="form.file" name="file"
