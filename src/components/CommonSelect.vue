        ...
        <VueMultiSelect v-model="value" :placeholder="placeholder" :options="options_handle" :class="[
            {
                'border-danger': validator && validator[name].$error
                    || collection_validator && (!props.collection_validator?.v$[props.collection_validator?.name].$each.$response.$errors[props.collection_validator?.index][props.name] && collection_validator.v$[collection_validator.name].$error) 
            },
