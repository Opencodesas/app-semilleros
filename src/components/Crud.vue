...
const managementAction = (id: string | number) => {
    setLoading(true)
    router.push({ name: 'documents.management', query: { contractor: id } }).finally(() => {
        setLoading(false)
    })
...
const contractorHandler = (name: string, id: string | number) => ({ name, query: { contractor: id } })
...
const hasDocumentsHeader = computed(() => props.headers.some((header) => header.value == 'documents' || header.value == 'contractor_documents'))
...
const contractorDocuments = computedAsync(async () => {
    if (hasDocumentsHeader.value) {
        ...
        return documents.data.map((document) => ({ id: document.contractor_id, status: document.status.slug }))
    }
}, null)

const contractorDocumentsCount = (id: number) => ((contractorDocuments.value != null) ? contractorDocuments.value.filter((doc) => doc.id == id).length : 0)
    ...
    <DataTable :headers="headers" :items="items" buttons-pagination :sort-by="sort.by" :sort-type="sort.type"
        ...
        <template #header-actionsDocuments="{ text }">
            ...
            <template #item-created_at="{ created_at }">
                <p>{{ (created_at != null && created_at != '') ? dayjs(created_at).format("MM/DD/YYYY") : '' }}</p>
            </template>
            ...
            <template #item-status="item">
                <span
                    :class="(item.status.slug == 'REC' || item.status.slug == 'NUL') ? ' bg-danger/10 text-danger' : (item.status.slug == 'COM' || item.status.slug == 'APR') ? 'bg-success/10 text-success' : 'bg-primary/10 text-primary'"
            ...
            <template #item-actions="item">
                <div class="flex gap-2 justify-end">
                    <template v-if="isProvider('assistants')">
                        <template v-if="hasDocumentsHeader && contractorDocuments != null">
                            <template v-if="contractorDocumentsCount(item.id) < 21">
                                <CommonButtonLink
                                    :to="{ name: 'assistants.contractorsUpload', query: { contractor: item.id } }"
                            ...
                            <template v-else-if="contractorDocumentsCount(item.id) >= 18 && item.status.slug == 'REC'">
                                <CommonButtonLink
                                    :to="{ name: 'assistants.contractorsControl', query: { contractor: item.id } }"
                                    variant="outline-secondary">
                    ...
                    <template v-else-if="isProvider('psychosocial-coordinator')">
                        <template v-if="true">
                            <Button v-if="item.status.slug == 'ENR'" variant="outline-secondary" @click="() => {
                                switch (selectedTab) {
                                    // case 1:
                                    //     router.push({ name: 'psychosocial.update', params: { id: item.id } })
                                    //     break;
                    ...
                    <template v-else-if="isProvider('technical_director') || isProvider('transversal_programs_director')">
                        <template v-if="true">
                            <Button variant="outline-secondary" @click="() => {
                                ...
                                isProvider('technical_director') ? router.push({ name: 'technical_director.visit-review', params: { id: item.id } }) : router.push({ name: 'transversal_programs_director.transversal-activity-review', params: { id: item.id } })
                    ...
                    <!-- <template v-else-if="isProvider('transversal_programs_director')">
                        <template v-if="true">
                            <Button variant="outline-secondary"
                                @click="() => { router.push({ name: 'transversal_programs_director.transversal-activity-review', params: { id: item.id } }) }">
                                <Lucide icon="View" class="mr-2" />
                                <span class="text-sm">
                                    Revisar
                                </span>
                    ...
                    <template v-else>
                        ...
                        <template v-if="item.status == 'APRO'">
                            ...
                            <Button variant="outline-success">
                                ...
                                <a class="text-sm" href="/Contrato.pdf" target="_blank">
                                    Contrato
                                </a>
                            </Button>
                        </template>
            ...
            <template #item-actionsDocuments="item">
                <div class="flex gap-2 justify-end">
                    ...
                    <template v-else-if="isProvider('legalMaster')">
                        <template v-if="item.status.slug == 'ENR'">
                            <CommonButtonLink :to="contractorHandler('legalMaster.contractsManagement', item.id)"
                                variant="outline-secondary">
                                <Lucide icon="FileDiff" class="mr-2" />
                                <span class="text-sm">
                                    Revision
            ...
            <template #item-actionsContracts="item">
                <div class="flex gap-2 justify-end">
                    <template v-if="isProvider('legal')">
                        ...
                        <template v-if="item.status.slug == 'REC'">
                            <CommonButtonLink
                                :to="{ name: 'legal.contractsClausesControl', query: { contractor: item.contractor_id } }"
                                variant="outline-secondary">
                                <Lucide icon="FileEdit" class="mr-2" />
                                <span class="text-sm">
                                    Revisar
                    ...
                    <template v-else-if="isProvider('legalMaster')">
                        <template v-if="item.status.slug == 'ENR'">
                            <CommonButtonLink
                                :to="{ name: 'legalMaster.contractsManagement', query: { contractor: item.id } }"
                                variant="outline-secondary">
                                <Lucide icon="FileDiff" class="mr-2" />
                                <span class="text-sm">
                                    Revision
                        ...
                        <template v-if="item.status.slug == 'APR'">
                            ...
                            <CommonButtonLink :to="{ name: 'legalMaster.contractsViewer', query: { id: item.id } }"
                                variant="outline-secondary">
                    ...
                    <template v-else-if="isProvider('director')">
                        ...
                        <template v-if="item.status.slug == 'APR'">
                            <CommonButtonLink :to="{ name: 'director.contractsManagement', query: { id: item.id } }"
                                variant="outline-secondary">
                                <Lucide icon="FileDiff" class="mr-2" />
                                <span class="text-sm">
                                    Revision
