		...
		await create("contracts-management", fd)
			.then((response) => {
				if (response.status >= 200 && response.status <= 300) {
					...
					alerts.custom(
						"",
						`El contrato de ${props.contractor.name} ${props.contractor.lastname} fue gestionado.`,
						"info"
					);
...
<template>
	<div class="flex justify-end">
		<Button type="button" variant="outline-primary" @click="openModal">
			...
			<span class="text-sm"> Gestionar </span>
		</Button>
	</div>
						...
						<DialogPanel
							...
							<form @submit.prevent="onSubmit" class="pt-2">
								...
								<div class="flex justify-end mt-4 space-x-4">
									...
									<Button type="submit" variant="soft-warning"> Enviar </Button>
								</div>
							</form>
						</DialogPanel>
					</TransitionChild>
